...
from ...models.autoencoders import AutoencoderKLCosmos
from ...models.controlnets.controlnet_cosmos import CosmosControlNetModel, CosmosMultiControlNetModel
from ...models.transformers import CosmosTransformer3DModel
...
class CosmosTransfer1Pipeline(DiffusionPipeline):
    r"""
    Pipeline for multimodal controllable conditional world generation using [Cosmos
    ...
    def __init__(
        ...
        controlnet: Union[
            CosmosControlNetModel, List[CosmosControlNetModel], Tuple[CosmosControlNetModel], CosmosMultiControlNetModel
        ],
        ...
        if isinstance(controlnet, (list, tuple)):
            controlnet = CosmosMultiControlNetModel(controlnet)
    ...
    def check_inputs(
        ...
        if [height, width] not in candidate_inputs:
            raise ValueError(f"(`height`, `width`) should be one of {candidate_inputs}")
        ...
        if image is None and video is None:
            if self.scheduler.config.sigma_max < 80:
                raise ValueError("Either `image` or `video` has to be provided when scheduler.config.sigma_max < 80")
    ...
    def __call__(
        ...
        # 6. Prepare ControlNet latent variables
        if isinstance(self.controlnet, CosmosControlNetModel):
            control_video = self.prepare_video(
                video=control_video,
                height=height,
                width=width,
                device=device,
            ...
            for control_video_ in control_video:
                control_video_ = self.prepare_video(
                    video=control_video_,
                    height=height,
                    width=width,
                    device=device,
                    dtype=vae_dtype,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    controlnet_block_samples_uncond = self.controlnet(
                        hint=control_video,
                        control_weight=control_weight,
                    ...
                    noise_pred_uncond = self.transformer(
                        ...
                        padding_mask=padding_mask,
                        block_controlnet_hidden_states=controlnet_block_samples_uncond,
                        return_dict=False,
                    )[0]
